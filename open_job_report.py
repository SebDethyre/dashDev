...
def get_last_completed_build_number(branch, appli):
    ...
    if response.status_code == 200:
        data = response.json()
        return data.get('lastCompletedBuild', {}).get('number')
        # return data.get('lastStableBuild', {}).get('number')
    else:
...
url = f"https://ci1.atreal.fr/view/P2%20devs/job/{application}__P2_{branch}/{build_number}/robot/report/log.html"
...
"""
   ...
   # Processus séparé en utilisant asyncio.create_subprocess_shell
